...
中心となる価値はシンプルです：中国語申請フォーム、ローカルプライバシー、デフォルトのローカルルール認識、任意の OpenAI-compatible API 認識。
...
- アカウント、プロジェクト提供バックエンド、テレメトリなし。API 認識はデフォルトで無効
- 動的ページ、モーダル、同一オリジン iframe に対応
- テキスト入力、ネイティブ select、カスタムドロップダウン、日付系入力に対応
- 説明可能なルールベースマッチング。任意の API 認識ではフィールドメタデータと利用可能な項目名のみを送り、保存済みプロフィール値は送信しません
